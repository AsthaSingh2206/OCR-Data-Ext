...
if uploaded_file and st.button('Process & Save to DB'):
    ...
    try:
        resp = requests.post(f'{st.secrets.get("API_URL", "http://127.0.0.1:5000")}/upload', files=files, data=params)
...
docs = []
try:
    docs = requests.get(f'{st.secrets.get("API_URL", "http://127.0.0.1:5000")}/docs').json()
except Exception as e:
    ...
    if selected_doc:
        ...
        with st.expander('Show Metadata & Preview', expanded=False):
            ...
            if st.button('Show JSON Preview'):
                doc_json = requests.get(f'{st.secrets.get("API_URL", "http://127.0.0.1:5000")}/doc/{doc_id}').json()
                st.json(doc_json)
else:
...
if st.button('Run Query'):
    if not user_sql.strip():
        ...
        resp = requests.post(f'{st.secrets.get("API_URL", "http://127.0.0.1:5000")}/query', json={'sql': user_sql})
        if resp.status_code == 200:
            data = resp.json()
